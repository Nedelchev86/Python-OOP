...
class BookstoreTests(TestCase):
    ...
    def test_books_limit_raise_error_value_zero(self):
    ...
    #
    #     self.assertEqual(f"Books limit of -3 is not valid", str(ve.exception))
    ...
    def test_receive_book_raise_exception(self):
        self.bookstore.availability_in_store_by_book_titles = {'first': 20, 'second': 70, 'third': 10}

        with self.assertRaises(Exception) as ex:
            self.bookstore.receive_book('first', 20)
    ...
    def test_receive_book_book_title_not_in_books(self):
        ...
        result2 = self.bookstore.receive_book("first", 10)

        self.assertEqual({'first': 30, 'second': 30, 'third': 10, 'random': 20}, self.bookstore.availability_in_store_by_book_titles)
        self.assertEqual(f"30 copies of first are available in the bookstore.", result)
    ...
    def test_sell_book_successfully(self):
        ...
        self.assertEqual({'first': 10, 'second': 30, 'third': 10}, self.bookstore.availability_in_store_by_book_titles)
        self.assertEqual(10, self.bookstore.total_sold_books)
        self.assertEqual(f"Sold 10 copies of first", result)

    def test_str_method(self):
        self.bookstore.availability_in_store_by_book_titles = {'first': 20, 'second': 30, 'third': 10}
        self.bookstore.sell_book('first', 10)
